...
*使用 jq("._scroll").yi_scroll();
*/


;(function(jq, window, document,undefined) {

jq.fn.yi_scroll = function(options)
{
	var scroll_config={
		"speed":25,
		"isMaxHeight":false,
		...
		"barColor" : "rgba(221,221,221,1)",
		"barMDColor" : "rgba(255,255,255,1)",
		"boxColor" : "rgba(255,255,255,1)",
		"isBox":false,
		"isBar":false
	}
	
	var opts = jq.extend({},scroll_config,options);
	...
	jq(document).on(sMouseWheel,function(ev){			/*处理区域滚动与文档默认滚动冲突*/
		if(w.MouseInScroll)
			ev.preventDefault();
	});
	...
	return this.each(function(){
		...
		elChild.css({"position":"absolute","top":"10px","width":"100%","left":"0"});

		if(opts.isMaxHeight)
		{
			var sham = jq("<div style='position:relative;background:transparent;z-index:-1;height:"+elChild.outerHeight()+"px'></div>");
			sham.appendTo(elem);
		}

		var jqScrollBox = jq("<div class='jqScroBox' style='position:absolute;opacity:"+(opts.isBox?1:1)+";width:"+opts.boxWidth+"px;top:"+opts.barOffTop+"px;right:"+opts.barOffRight+"px;bottom:"+opts.barOffBottom+"px;border-radius: 5px;background: "+color.box+";'></div>");
		jqScrollBox.addClass(opts.boxClass);
		var jqScrollBar= jq("<div style='position:absolute;opacity:"+(opts.isBar?1:1)+";width:"+opts.barWidth+"px;top:"+opts.barOffTop+"px;right:"+opts.barOffRight+"px;border-radius: 5px;background: "+color.bar+";'></div>");
		...
		jq(document).on("mousedown",function(ev){
			if(isS_B)
				doc_py = ev.pageY;
		});
		jq(document).on("mousemove",function(ev){
			if(isS_B)
			{			
		...
		jq(document).on("mouseup",function(ev){	
